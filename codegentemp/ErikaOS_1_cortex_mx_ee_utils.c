...
#if (Number_of_ISRs > 0) && defined isr_start_game__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_start_game__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_start_game;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_start_game__INTC_NUMBER, (ISR_MIN_PRI - 4));
    CyIntSetPriority(isr_start_game__INTC_NUMBER, ISR_SCALE(4));
    //\PF
	NVIC_INT_ENABLE(isr_start_game__INTC_NUMBER);
#elif Number_of_ISRs > 0
#error  The Interrupt isr_start_game could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_start_game */

#if (Number_of_ISRs > 1) && defined isr_button_press__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_button_press__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_button_press;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_button_press__INTC_NUMBER, (ISR_MIN_PRI - 3));
    CyIntSetPriority(isr_button_press__INTC_NUMBER, ISR_SCALE(3));
    //\PF
	NVIC_INT_ENABLE(isr_button_press__INTC_NUMBER);
#elif Number_of_ISRs > 1
#error  The Interrupt isr_button_press could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_button_press */

#if (Number_of_ISRs > 2) && defined isr_3secs__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_3secs__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_3secs;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_3secs__INTC_NUMBER, (ISR_MIN_PRI - 4));
    CyIntSetPriority(isr_3secs__INTC_NUMBER, ISR_SCALE(4));
    //\PF
	NVIC_INT_ENABLE(isr_3secs__INTC_NUMBER);
#elif Number_of_ISRs > 2
#error  The Interrupt isr_3secs could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_3secs */

#if (Number_of_ISRs > 3) && defined isr_5msec__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_5msec__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_5msec;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_5msec__INTC_NUMBER, (ISR_MIN_PRI - 5));
    CyIntSetPriority(isr_5msec__INTC_NUMBER, ISR_SCALE(5));
    //\PF
	NVIC_INT_ENABLE(isr_5msec__INTC_NUMBER);
#elif Number_of_ISRs > 3
#error  The Interrupt isr_5msec could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_5msec */
